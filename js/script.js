...
const calculateItemPrice = (price, count) => `${price * count}$`;
...
goods.forEach((item) => {
  item.total = `${calculateItemPrice(item.price, item.count)}`;
});

const createRow = (item) => {
  const {id, title, category, units, count, price, total, images} = item;
  ...
  const cells = [
    ...
    {className: 'table__cell-tbody table__cell-total', value: total},
    {
